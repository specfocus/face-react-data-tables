...
export const addOneRecord = (
    ...
    return <any>Object.defineProperty(newRecordsById, 'fetchedAt', {
        value: { ...oldRecords.fetchedAt, [newRecord.id]: date },
        enumerable: false,
    });
};

const includesNotStrict = (items: any[], element: string) =>
    items.some((item: any) => item == element); // eslint-disable-line eqeqeq
...
const dataReducer: Reducer<RecordSetWithDate> = (
    ...
    if (meta && meta.optimistic) {
        ...
        if (meta.fetch === UPDATE_MANY) {
            const updatedRecords = payload.ids.map((id: string | number) => ({
                ...previousState[id],
                ...payload.data,
            }));
...
export const getRecord = (state: { [x: string]: any; }, id: string | number) => state[id];
