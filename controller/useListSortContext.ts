...
const useListSortContext = (props?: any): ListSortContextValue => {
    ...
    if (!context.setSort) {
         ...
         *
         * This may only happen when using DataView / SimpleList / SingleFieldList components
        ...
        if (process.env.NODE_ENV !== 'production') {
            console.log(
                "List components must be used inside a <ListContextProvider>. Relying on props rather than context to get List data and callbacks is deprecated and won't be supported in the next major version of ../../app."
            );
        }
        return props;
    }
